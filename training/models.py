...
'''multimodel DL model for sentiment and emotion classification
Using Text BERT embedding, Video ResNet3D, Audio ID CNN.
Encoders are used for text, video and audio.
Fusion layer to combine the embeddings
Classification heads for emotion and sentiment
Trainer class to handler training and eval'''

class TextEncoder(nn.Module):
    '''BERT embeddings, and freezes them
    Reduces dimensions from 768 to 128 using nn.Linear()
    Returns a final text embedding'''
    def __init__(self):
        super().__init__()
...
class VideoEncoder(nn.Module):
    '''Uses ResNet3D for video emcoding, freezing the pre-tuned weights
    Reduces video features to 128dim
...
class AudioEncoder(nn.Module):
    '''ID CNN for audio feature extraction from mel specto
    '''
    def __init__(self):
        super().__init__()
...
if __name__ == "__main__":
    ...
    # dataset = MELDDataset(train_file, train_splits)
